...
contract VoteContract4_yul {
    ...
    function TotalVoters(address[] memory arrayOfVotersC) public pure returns (uint256 length) {
        assembly {
            length := mload(arrayOfVotersC)
        }
    }

    function TotalPositiveVoters(address[] calldata arrayOfVotersC) public view returns (uint256 sum) {
    ...
    function TotalNegativeVoters(address[] calldata arrayOfVotersC) public view returns (uint256 sum) {
        for (uint256 i = 0; i < arrayOfVotersC.length; i++) {
            if (!votingResult[arrayOfVotersC[i]]) {
                assembly {
                    sum := add(sum, 1)
