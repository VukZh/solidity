...
describe("test vote contract 5", () => {
  it("test initial state", async () => {
    const vc = await ethers.deployContract("VoteContract5_optimize");
  ...
  it("test make a vote", async () => {
    ...
    expect(await vc.TotalNegativeVoters()).to.equal(BigInt(1n));
  ...
  it("test make an incorrect vote", async () => {
    ...
    expect(await vc.TotalNegativeVoters()).to.equal(BigInt(2));
