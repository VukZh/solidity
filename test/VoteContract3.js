...
describe("test vote contract 3", () => {
  it("test initial state", async () => {
    const vc = await ethers.deployContract("VoteContract3_calldata");
  ...
  it("test make a vote", async () => {
    ...
    expect(await vc.TotalNegativeVoters([...arrOfVoters])).to.equal(1);
  ...
  it("test make an incorrect vote", async () => {
    ...
    expect(await vc.TotalNegativeVoters([...arrOfVoters])).to.equal(2);
